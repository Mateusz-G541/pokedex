...
test.describe('Random Pokemon Functionality', () => {
  ...
  test('should generate random Pokemon from Generation 1 only (IDs 1-151)', async ({ 
    ...
    expect(
      uniqueIds.size,
      'Should generate different Pokemon (randomness check)',
    ).toBeGreaterThan(1);
  ...
  test('should display valid Pokemon data when using random function', async ({ 
    page: _page,
  }) => {
    ...
    expect(
      pokemonId,
      'Pokemon ID should be within Generation 1 range',
    ).toBeGreaterThanOrEqual(1);
    expect(
      pokemonId,
      'Pokemon ID should be within Generation 1 range',
