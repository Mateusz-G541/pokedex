...
interface EvolutionChainData {
  ...
  chain: {
    ...
    evolves_to: Array<EvolutionChainData['chain']>;
  };
}
...
class PokemonDataScraper {
  ...
  async scrapePokemonData(): Promise<void> {
    ...
    const POKEMON_LIMIT = 151; // Change this to totalPokemon for full scrape
    ...
    for (let i = 1; i <= Math.min(POKEMON_LIMIT, totalPokemon); i++) {
      try {
        ...
        if (pokemon.sprites.other['official-artwork'].front_default) {
          ...
          pokemon.sprites.other['official-artwork'].front_default =
            `/images/pokemon/artwork/${filename}`;
        }
    ...
    // Save Pokemon data
    fs.writeFileSync(path.join(this.dataDir, 'pokemon.json'), JSON.stringify(pokemonData, null, 2));

    // Save species data
    fs.writeFileSync(path.join(this.dataDir, 'species.json'), JSON.stringify(speciesData, null, 2));
  ...
  async scrapeTypeData(): Promise<void> {
    ...
    // Save type data
    fs.writeFileSync(path.join(this.dataDir, 'types.json'), JSON.stringify(typeData, null, 2));
